...
# GROUND STATION: FATİH KÜÇÜKBIYIK
# AUTONOMOUS FLIGHT CODE: EDANUR KUTAY
# IMAGE PROCESSING: ALPEREN DAŞ
...
class DestekThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            while destek_iha is None or veriler is None:
            ...
            arm_message = dialect.MAVLink_command_long_message(
                destek_iha.target_system, destek_iha.target_component,
                dialect.MAV_CMD_COMPONENT_ARM_DISARM, 0, 1, 0, 0, 0, 0, 0, 0
            )
            destek_iha.mav.send(arm_message)
            time.sleep(2)  # Arm komutunun işlenmesi için bekleme

            # GUIDED moduna geçme
            destek_iha.set_mode("GUIDED")
            time.sleep(2)  # Mod değişikliği için bekleme

            # Takeoff komutu gönderme
            takeoff_command = dialect.MAVLink_command_long_message(
                destek_iha.target_system, destek_iha.target_component,
                dialect.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, 10
            )
            destek_iha.mav.send(takeoff_command)
            time.sleep(10)  # Kalkışı tamamlaması için bekleme

            # Görev öğesini oluşturma
            message = dialect.MAVLink_mission_item_int_message(destek_iha.target_system, destek_iha.target_component, 0,
            ...
            destek_iha.waypoint_clear_all_send()
            time.sleep(2)
            destek_iha.waypoint_count_send(1)
            time.sleep(2)
            destek_iha.mav.send(message)
            time.sleep(30)
            destek_iha.set_mode("QLAND")

        except Exception as err:
            print(f"Destek Thread Error. {err}")
...
class DestekDonusThread(QThread):
    def run(self):
        ...
        try:
            ...
            # Görev öğesini oluşturma
            message = dialect.MAVLink_mission_item_int_message(destek_iha.target_system, destek_iha.target_component,
...
class ImhaThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            def calculate_new_locations(distance_m, bearing_deg, start_lat, start_lon):
                R = 6371000  # Dünya yarıçapı (metre cinsinden)
                bearing_rad = math.radians(bearing_deg)  # Dereceden radiana çevirme
                start_lat_rad = math.radians(start_lat)
                start_lon_rad = math.radians(start_lon)

                # Yeni koordinatları hesaplama
                end_lat_rad = math.asin(math.sin(start_lat_rad) * math.cos(distance_m / R) +
                                        math.cos(start_lat_rad) * math.sin(distance_m / R) * math.cos(bearing_rad))
                end_lon_rad = start_lon_rad + math.atan2(
                    math.sin(bearing_rad) * math.sin(distance_m / R) * math.cos(start_lat_rad),
                    math.cos(distance_m / R) - math.sin(start_lat_rad) * math.sin(end_lat_rad))

                # Radian'dan dereceye çevirme
                end_lat = math.degrees(end_lat_rad)
                end_lon = math.degrees(end_lon_rad)

                return end_lat, end_lon

            # Yeni konumları hesaplama ve ekleme
            directions = ["kuzeydogu", "guneydogu", "guneybatı", "kuzeybatı"]
            bearings = [45, 135, 225, 315]

            for i, direction in enumerate(directions):
                bearing = bearings[i]
                lat, lon = calculate_new_locations(125, bearing, TARGET_LOCATIONS[0]["latitude"],
                                                   TARGET_LOCATIONS[0]["longitude"])
                TARGET_LOCATIONS.append({"location_name": direction, "latitude": lat, "longitude": lon, "altitude": 10})

            TARGET_LOCATIONS.append(
                {"location_name": "target_location", "latitude": -35.35985798, "longitude": 149.16399990,
                 "altitude": 10})
            TARGET_LOCATIONS.append(
                {"location_name": "home_location", "latitude": -35.36325792, "longitude": 149.16523471, "altitude": 10})
            ...
            # Arm komutu gönderme
            arm_message = dialect.MAVLink_command_long_message(
                imha_iha.target_system, imha_iha.target_component,
                dialect.MAV_CMD_COMPONENT_ARM_DISARM, 0, 1, 0, 0, 0, 0, 0, 0
            )
            imha_iha.mav.send(arm_message)
            time.sleep(2)  # Arm komutunun işlenmesi için bekleme

            # GUIDED moduna geçme
            imha_iha.set_mode("GUIDED")
            time.sleep(2)  # Mod değişikliği için bekleme
            ...
            imha_iha.mav.send(takeoff_command)
            time.sleep(10)  # Kalkışı tamamlaması için bekleme

            # Görev öğelerini oluşturma ve gönderme
            message = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                               0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                               dialect.MAV_CMD_NAV_WAYPOINT,
                                                               2, 0, 0, 0, 0, 0,
                                                               x=int(TARGET_LOCATIONS[0]["latitude"] * 1e7),
            ...
            message1 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[1]["latitude"] * 1e7),
            ...
            message2 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[2]["latitude"] * 1e7),
                                                                y=int(TARGET_LOCATIONS[2]["longitude"] * 1e7),
                                                                z=TARGET_LOCATIONS[2]["altitude"]
                                                                )

            message3 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[3]["latitude"] * 1e7),
                                                                y=int(TARGET_LOCATIONS[3]["longitude"] * 1e7),
                                                                z=TARGET_LOCATIONS[3]["altitude"]
                                                                )

            message4 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[4]["latitude"] * 1e7),
                                                                y=int(TARGET_LOCATIONS[4]["longitude"] * 1e7),
                                                                z=TARGET_LOCATIONS[4]["altitude"]
                                                                )

            message5 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[5]["latitude"] * 1e7),
                                                                y=int(TARGET_LOCATIONS[5]["longitude"] * 1e7),
                                                                z=TARGET_LOCATIONS[5]["altitude"]
                                                                )

            message6 = dialect.MAVLink_mission_item_int_message(imha_iha.target_system, imha_iha.target_component,
                                                                0, dialect.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                                                                dialect.MAV_CMD_NAV_WAYPOINT,
                                                                2, 0, 0, 0, 0, 0,
                                                                x=int(TARGET_LOCATIONS[6]["latitude"] * 1e7),
                                                                y=int(TARGET_LOCATIONS[6]["longitude"] * 1e7),
                                                                z=TARGET_LOCATIONS[6]["altitude"]
                                                                )

            imha_iha.waypoint_clear_all_send()
            time.sleep(2)
            imha_iha.waypoint_count_send(5)  # Görev sayısını dinamik olarak ayarlama
            time.sleep(2)

            # Her görevi ayrı ayrı gönderme

            imha_iha.mav.send(message)
            time.sleep(30)
            imha_iha.mav.send(message1)
            time.sleep(30)
            imha_iha.mav.send(message2)
            time.sleep(30)
            imha_iha.mav.send(message3)
            time.sleep(30)
            imha_iha.mav.send(message4)
            time.sleep(30)
            imha_iha.mav.send(message5)
            time.sleep(50)
            imha_iha.mav.send(message6)
            time.sleep(50)
            imha_iha.set_mode("QLAND")
        except Exception as err:
            print(f"Imha Thread Error: {err}")
        self.finished.emit()
        iha_gorevde = False
